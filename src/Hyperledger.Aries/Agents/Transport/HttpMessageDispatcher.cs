        ...
        public async Task<PackedMessageContext> DispatchAsync(Uri endpointUri, PackedMessageContext message)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                throw new AriesFrameworkException(
                    ErrorCode.A2AMessageTransmissionError, $"Failed to send A2A message with an HTTP status code of {response.StatusCode} and content {responseBody}",message.ContextRecord);
            }
